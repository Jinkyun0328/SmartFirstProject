# SmartFirstProject
## 프로젝트 개요
![images1](https://user-images.githubusercontent.com/123911778/269862986-61a7269d-3862-4a1d-80da-0757a6433b28.jpg)
주제 : 데이터베이스를 활용한 카지노 게임
- 회원가입, 로그인, 회원탈퇴 서비스 제공
- 돈을 기준으로 한 개인조회와 전체조회 서비스 제공
- 블랙잭, 룰렛, 슬롯머신의 게임 서비스 제공

## 개발환경
분류 | 설명
---- | ----
운영체제 | Windows 10
버전 관리 시스템 | Git, Github
개발 도구 | Eclipse IDE (2023-06), JavaSE-1.8
개발언어 | Java, SQL
데이터베이스 | ORACLE 11
하드웨어 |  NVIDIA GeForce RTX 2070, Intel(R) Core(TM) i7-10700 CPU, RAM 16.0GB

## 시스템 설계도
![images5](https://user-images.githubusercontent.com/123911778/269863042-80e6fcc7-9523-4a8c-82ff-9cc1a3493f1c.jpg)
![images6](https://user-images.githubusercontent.com/123911778/269863045-ba8527fa-dc0d-4dee-8a95-5ddf5f3b8fb8.jpg)
![images7](https://user-images.githubusercontent.com/123911778/269863046-6055a2e1-2fd4-404b-834a-db9eb2d41e62.jpg)

## 테스트
![images8](https://user-images.githubusercontent.com/123911778/269863053-83df96d8-2cad-4137-b125-5c23952a8a48.jpg)

## 기타
### 참여인원
이름 | 역할
---- | ----
김성민(팀장) | 기획 및 게임 알고리즘 설계
강채린(부팀장) | 효과음 수집 및 mp3 모델 설계
박준 | 콘솔아트를 통한 그래픽 요소 설계
송희주 | DB 연동 및 데이터 전송 모델 설계
최진균 | 게임 알고리즘 설계 및 코드 통합


### 데이터
폴더 | 내용
---- | ----
FINAL | exe파일, 음악파일, 산출물 등을 저장
SAVE | 날짜별 프로젝트 백업 저장
Development Log | 개발일지 저장
project | 최종 자바 프로젝트 저장

